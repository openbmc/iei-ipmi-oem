                     ...
                     const std::string& interface, ObjectTree& objectTree)
{
    ...
    if (ec)
    {
        lg2::error("getAllDbusObjects for (path:{ROOTPATH}, intf: {INTF}) "
                   "failed with {ERRMSG}",
                   "ROOTPATH", serviceRoot, "INTF", interface, "ERRMSG",
                   ec.message());
              ...
              const std::string& path, std::string& service)
{
    ...
    if (ec)
    {
        lg2::error("getService for (path:{PATH},interface:{INTERFACE}) "
                   "failed with {ERRMSG}",
                   "PATH", path, "INTERFACE", interface, "ERRMSG",
                   ec.message());
        return ccUnspecifiedError;
    }
                   ...
                   const std::string property, const Value& value)
{
    ...
    if (ec)
    {
        lg2::error("setDbusProperty for (service:{SERVICE}, path:{PATH}, "
                   "interface:{INTERFACE}) "
                   "failed with {ERRMSG}",
                        ...
                        const std::string& interface, PropertyMap& properties)
{
    ...
    if (ec)
    {
        lg2::error("getAllDbusProperties for (service:{SERVICE}, path:{PATH}, "
                   "interface:{INTERFACE}) "
                   "failed with {ERRMSG}",
                   "SERVICE", service, "PATH", objPath, "INTERFACE", interface,
                   "ERRMSG", ec.message());
